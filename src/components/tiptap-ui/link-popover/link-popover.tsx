...
>(
  (
    ...
    const handleSetLink = React.useCallback(() => {
      ...
      // Update the state with the corrected URL before setting it
      setUrl(finalUrl); 
    
      // onSetLink is a more direct way to apply the link from the hook's perspective
      if (onSetLink) {
          onSetLink(finalUrl);
      } else {
          // Fallback to the setLink from the hook if onSetLink is not provided
          setLink();
      }
    
      setIsOpen(false);
    }, [url, setUrl, setLink, removeLink, onSetLink]);
