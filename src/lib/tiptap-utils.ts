...
export const handleImageUpload = async (
  ...
  try {
    ...
    // The editor's Image extension expects a full URL.
    // This returns the relative path which will work correctly.
    return result.path;
  } catch (error: any) {
    console.error('Tiptap image upload error:', error);
    toast.error(error.message || 'Image upload failed.');
    // Throw error to be caught by the editor's onError handler
    throw error;
  }
}
